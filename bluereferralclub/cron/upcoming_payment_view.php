...
try {
    ...
    foreach ($users as $user) {
        ...
        // Nome da view específica para status 'Successes'
        $viewName = strtolower("{$firstName}{$refCode}upcoming_payment_view");

        // Apaga a view se já existir
        $dropViewSQL = "DROP VIEW IF EXISTS `$viewName`";
        $pdo->exec($dropViewSQL);

        // Cria nova view com total de comissões com status 'Successes'
        $createViewSQL = "
            ...
            FROM referrals
            WHERE user_id = $userId AND status = 'Successes';
        ";
