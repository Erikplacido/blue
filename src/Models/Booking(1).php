...
class Booking {
    ...
    public function markAsPaid(int $bookingId): void {
        $stmt = $this->db->prepare("UPDATE bookings SET status = 'paid' WHERE id = ?");
    ...
    public function markAsFailed(int $bookingId): void {
        $stmt = $this->db->prepare("UPDATE bookings SET status = 'failed' WHERE id = ?");
        $stmt->execute([$bookingId]);
    }
...
public function updateBreakFee(int $bookingId, float $breakFee): bool
{
     ...
     *
     * @param int    $bookingId
     * @param string $status      'active', 'paused' ou 'canceled'
