...
$recurrence = in_array($input['recurrence'], $validRecurrences) ? $input['recurrence'] : 'one-time';

switch ($recurrence) {
    case 'weekly':      $interval = 'P7D';  break;
    case 'fortnightly': $interval = 'P15D'; break;
    case 'monthly':     $interval = 'P30D'; break;
...
$discountAmount = 0;
$couponCode = trim($input['discountCode'] ?? '');
$validCouponCode = '';
...
if ($couponCode !== '') {
    ...
    if ($coupon) {
        ...
        die('The discount code "' . htmlspecialchars($couponCode) . '" is invalid or inactive.');
    }
}
...
// 16. Atualizar ID da assinatura (se aplicável)
